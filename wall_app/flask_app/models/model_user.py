...
class User:
    ...
    @classmethod
    def create(cls, data):
        query = query = "INSERT INTO users (first_name,last_name,email,pw,created_at,updated_at) VALUES(%(first_name)s,%(last_name)s,%(email)s,%(pw)s,NOW(),NOW())"
    ...
    def validator_login(form_data):
        ...
        if len(form_data['pw']) <8 :
            is_valid= False
            flash("Password length must be 8 or more characters", "err_user_pw_login")
            # session['err_pw']= "Last Name Required"
